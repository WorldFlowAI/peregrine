/*
 * peregrine - checkasm module for the f32 elementwise multiply.
 * Each output is a single rounded product, identical in C and SIMD: exact.
 */
#include "checkasm.h"
...
static void fuzz_variant(const PgMulVariant *v, float *a, float *b, float *r, float *t)
{
    ...
    for (int it = 0; it < 420 && ok; it++) {
        ...
        pg_mul_f32_c(a + off, b + off, r + off, n);
        CK_CALL_BINOP(v->fn, a + off, b + off, t + off, n);

        const char *reg;
        if (checkasm_clobbered(&reg)) {
            checkasm_fail("mul.%s n=%zu off=%zu clobbered callee-saved %s", v->name, n, off, reg);
            ok = 0;
        } else if (memcmp(r + off, t + off, n * sizeof(float)) != 0) {
            checkasm_fail("mul.%s n=%zu off=%zu mag=%g mismatch", v->name, n, off, mag);
            ok = 0;
        }
    }
    checkasm_report("mul_f32", v->name, ok);
}

static double bench_variant(pg_mul_f32_fn fn, const float *a, const float *b, float *o, size_t n, double *gbps)
{
    int iters = 1;
    double el = 0.0;
    fn(a, b, o, n);
...
{
    size_t nv;
    const PgMulVariant *v = pg_mul_variants(&nv);
    ...
    if (checkasm_bench_enabled()) {
        ...
        for (size_t s = 0; s < sizeof sizes / sizeof sizes[0]; s++) {
            ...
            char title[48];
            snprintf(title, sizeof title, "mul_f32  n=%zu", n);
