/*
 * peregrine - checkasm module for the f32 max reduction (n >= 1).
 * max picks an actual input element, so the result is exact: ref == got.
 */
#include "checkasm.h"
...
static void fuzz_variant(const PgMaxVariant *v, float *x)
{
    ...
    for (int t = 0; t < 420 && ok; t++) {
        ...
        float ref = pg_max_f32_c(p, n);
        float got = CK_CALL_REDUCE(v->fn, p, n);

        const char *reg;
        if (checkasm_clobbered(&reg)) {
            checkasm_fail("max.%s n=%zu off=%zu clobbered callee-saved %s", v->name, n, off, reg);
            ok = 0;
        } else if (ref != got) {
            checkasm_fail("max.%s n=%zu off=%zu mag=%g ref=%.6g got=%.6g", v->name, n, off, mag, ref, got);
            ok = 0;
        }
    }
    checkasm_report("max_f32", v->name, ok);
}

static double bench_variant(pg_max_f32_fn fn, const float *x, size_t n, double *gbps)
{
    ...
    (void)sink;
    double per = el / iters;
    *gbps = (double)n * sizeof(float) / per / 1e9;
    return per;
}
...
{
    size_t nv;
    const PgMaxVariant *v = pg_max_variants(&nv);
    ...
    if (checkasm_bench_enabled()) {
        ...
        for (size_t s = 0; s < sizeof sizes / sizeof sizes[0]; s++) {
            ...
            char title[48];
            snprintf(title, sizeof title, "max_f32  n=%zu", n);
